...
func TestVaultSource(t *testing.T) {
	...
	vault := exec.Command("vault", "server", "-dev", "-dev-root-token-id="+token, "-dev-listen-address="+addr)
	...
	if err != nil {
		t.Fatalf("api.NewClient failed: %s", err)
	}
	c.SetToken(token)

	waitForVault := func() chan bool {
		ch := make(chan bool)
		go func() {
			for {
	...
	testSource(t, VaultSource{Addr: "http://" + addr, token: token, CertPath: certPath}, makeCertPool(certPEM), 50*time.Millisecond)
}
...
func testSource(t *testing.T, source Source, rootCAs *x509.CertPool, sleep time.Duration) {
	...
	call := func(host string) (statusCode int, body string, err error) {
		// we need to call https://localhost:xxxxx/
		// Setting the Host header to provide the hostname does not work.
	...
	// calling https://<hostname>/ should fail since there is only a cert for localhost
